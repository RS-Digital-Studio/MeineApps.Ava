...
namespace ArcaneKingdom.Domain.Tests
{
    ...
    public sealed class GuildTests
    {
        ...
        {
            Assert.Throws<ArgumentException>(() => new GuildSnapshot("id", "TestName", "ABCD", "leader", DateTime.UtcNow));
            Assert.Throws<ArgumentException>(() => new GuildSnapshot("id", "TestName", "ABCDEF", "leader", DateTime.UtcNow));
            Assert.DoesNotThrow(() => new GuildSnapshot("id", "TestName", "ABCDE", "leader", DateTime.UtcNow));
        }

        [Test]
        public void GuildNameMussZwischen3Und20ZeichenSein()
        {
            Assert.Throws<ArgumentException>(() => new GuildSnapshot("id", "AB", "ABCDE", "leader", DateTime.UtcNow));
            Assert.Throws<ArgumentException>(() => new GuildSnapshot("id", new string('X', 21), "ABCDE", "leader", DateTime.UtcNow));
            Assert.DoesNotThrow(() => new GuildSnapshot("id", "ABC", "ABCDE", "leader", DateTime.UtcNow));
        }

        [Test]
        public void MaxMembersSkaliertMitLevel()
        {
            var g = new GuildSnapshot("id", "TestName", "ABCDE", "leader", DateTime.UtcNow);
        ...
        public void ContributionRequiredFuerLevelSteigtMonoton()
        {
            ...
            {
                var req = GuildSnapshot.ContributionRequiredForLevel(lv);
