...
`MenuBackgroundCanvas` weicht ab: erbt von `UserControl`, baut intern einen `SKCanvasView` als `Content`, implementiert zusätzlich `IsEffectivelyVisible`-Prüfung im Timer-Tick, damit inaktive Tabs keinen Frame erzeugen.
