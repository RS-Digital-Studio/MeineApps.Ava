...
**Version:** 2.0.23 (VersionCode 33) | **Package-ID:** org.rsdigital.bomberblast | **Status:** Geschlossener Test
...
- GC-Optimierung: Gepoolte SKPaint/SKFont/SKPath (fast alle per-frame Allokationen eliminiert), HUD-String-Caching (11 Cache-Felder: Survival-Timer, Combo, Enemies, Speed, Curse), gecachter SKMaskFilter für HUD-Glow. Statische Cleanup()-Methoden in ExplosionShaders, MenuBackgroundRenderer, HelpIconRenderer für Dispose bei App-Shutdown
- **SKPath-Pooling**: _charPath1/_charPath2 (Characters), _bgPath (Atmosphere+Grid), _irisClipPath/_starPath (Render), _torchPath (DynamicLighting), _tempPath (Weather+Ambient), _poolPath1/_poolPath2 (MenuBackground). Alle mit Rewind() wiederverwendet (Reset() nur bei FillType-Wechsel in Grid.Fog)
- **Shader-Optimierung**: Alle per-Frame SKShader-Allokationen eliminiert. Background/Vignette/DynamicLighting-Shader beim Init gecacht. Grid-Border-Transitions (Ice/Lava/Teleporter) nutzen 2-Step-Alpha-DrawRect statt LinearGradient. ExplosionShaders DrawCenterFire nutzt Solid-Color+MaskFilter statt 3x RadialGradient. Fog-Overlay nutzt Stroke-Ringe statt RadialGradient. DynamicLighting: 3 statische MaskFilter-Tiers (_blurSmall/Medium/Large) statt pro-Licht Allokation. MenuBackground: Gradient-Shader gecacht (invalidiert bei Resize/Theme-Wechsel), Vignette via Solid-Color+MaskFilter. ShaderEffects: SKRuntimeEffectUniforms + float[]-Arrays gecacht
- **EnemyPositionCache**: Lists werden via Clear() wiederverwendet statt Dict.Clear()+new List, periodischer Cleanup alle 120 Frames
...
| HelpIconRenderer | Statische Enemy/Boss/PowerUp/BombCard Icons für HelpView, CollectionView, DeckView. DrawEnemy (12 Typen), DrawBoss (5 Typen), DrawPowerUp (12 Typen), DrawBombCard (14 Typen mit Farben aus GameRenderer.Items.cs). Gepoolte SKPaint (_fillPaint/_strokePaint) + gecachte SKMaskFilter (_bombGlowLarge/_bombGlowSmall/_sparkGlow), Cleanup() |
...
### Input-Handler (3x)
- **FloatingJoystick**: Touch-basiert, zwei Modi: Floating (erscheint wo getippt, Standard) + Fixed (immer sichtbar unten links). Bomb-Button weiter in die Spielfläche gerückt (80px/60px Offset statt 30px/20px). Mobile-optimierte Sizes: Joystick-Radius 75dp, Deadzone Fixed 15% / Floating 5%, Bomb-Button 50dp, Detonator 48dp, Hit-Zone 1.6x Visual. Richtungs-Hysterese (1.15x) gegen Flackern bei ~45°
...
- **01.03.2026**: **Immersiver Ladebildschirm**: Loading-Pipeline (`Loading/BomberBlastLoadingPipeline.cs`) mit `ShaderPreloader` + `ExplosionShaders.Preload()` parallel (weight 40) + ViewModel-Erstellung (weight 20). `App.axaml.cs` nutzt `Panel(MainView + SkiaLoadingSplash)`-Pattern mit `RunLoadingAsync`. `DataContext` wird erst nach Pipeline-Abschluss gesetzt (nicht mehr synchron beim Start). Partikel-Effekte via `SplashScreenRenderer` aus `MeineApps.UI`.
- **28.02.2026 (44)**: **Erweitertes Preloading im SplashOverlay**: PreloadAction von 1 auf 2 Schritte erweitert. Schritt 1: ShaderPreloader.PreloadAll() (12 SkSL-Shader). Schritt 2: 5 statische Renderer-Klassen vorladen (ExplosionShaders.Preload() inkl. Noise-LUT, HelpIconRenderer.Preload(), TornMetalRenderer.Preload(), RarityRenderer.Preload(), MenuBackgroundRenderer.Preload()). Alle Preload()-Methoden triggern den CLR-Klassen-Initializer → statische SKPaint/SKFont/SKMaskFilter/SKPath-Felder werden vorab allokiert. ExplosionShaders.Preload() ruft zusätzlich InitNoise() auf (512-Element Float-Array). Build 0 Fehler.
- **28.02.2026 (43)**: **SplashOverlay / Ladebildschirm**: SplashOverlay (MeineApps.UI.Controls) in MainView.axaml integriert (Panel-Wrapper um Root-Grid + SplashOverlay darüber). PreloadAction in MainView.axaml.cs OnAttachedToVisualTree: ShaderPreloader.PreloadAll() (alle 12 SkSL-Shader vorkompilieren). Funktioniert auf Desktop UND Android (MainView ist Shared, nicht MainWindow). Build 0 Fehler.
- **28.02.2026 (42)**: **SkiaSharp Cleanup + Allokations-Optimierung**: (1) **ExplosionShaders.cs**: Statische Cleanup()-Methode für _flamePaint/_softGlow/_mediumGlow/_armPath, Thread-Safety-Kommentar. (2) **MenuBackgroundRenderer.cs**: Statische Cleanup()-Methode für 11 statische native Objekte (_gradientPaint/_gridPaint/_p1-p3/_strokePaint/_smallGlow/_mediumGlow/_largeGlow/_poolPath1/_poolPath2 + _cachedGradientShader), Thread-Safety-Kommentar. (3) **HelpIconRenderer.cs**: Per-Call `using var` SKPaint-Allokationen (DrawEnemy/DrawPowerUp/DrawBoss/DrawBombCard) durch statische gecachte _fillPaint/_strokePaint ersetzt. 3 per-Call SKMaskFilter.CreateBlur() durch gecachte _bombGlowLarge/_bombGlowSmall/_sparkGlow ersetzt. Cleanup()-Methode. (4) **GameRenderer.Dispose()**: Verifiziert - alle IDisposable-Felder (20+ SKPaint/SKFont/SKPath/SKMaskFilter/SKShader/SKBitmap + 5 Subsysteme) korrekt in Dispose() freigegeben. (5) **Englische Kommentare**: Alle 8 geänderten Dateien geprüft - bereits alle deutsch. Build 0 Fehler.
