...
public class ExportService : IExportService
{
    ...
    private async Task<string> ExportStatisticsToPdfInternal(string period, DateTime? startDate, DateTime? endDate, string? targetPath = null)
    {
        ...
        // Fußzeile
        var footerText = $"{_localizationService.GetString("GeneratedBy") ?? "Generated by"} FinanzRechner - {DateTime.UtcNow.ToString("g", System.Globalization.CultureInfo.CurrentUICulture)}";
        ...
        {
            var exportDir = _fileShareService.GetExportDirectory("FinanzRechner");
            filePath = Path.Combine(exportDir, $"statistics_{DateTime.UtcNow:yyyyMMdd_HHmmss}.pdf");
        }
        try
        {
