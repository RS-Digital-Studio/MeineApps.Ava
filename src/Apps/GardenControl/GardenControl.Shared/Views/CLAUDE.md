...
| `SettingsView.axaml(.cs)` | Einstellungen: Server-URL-Eingabe, Verbindungstest, Server-Info. |
...
- **CalibrationView.OnLoaded:** Ruft `LoadZonesCommand` erst beim ersten Sichtbarwerden auf —
  nicht im ViewModel-Konstruktor, weil die Serververbindung beim Start noch nicht steht.
  Das ist eine bewusste Ausnahme vom "kein Event-Handler im Code-Behind"-Prinzip. Das Event
  ist kein Geschäftslogik-Event, sondern ein reiner View-Lifecycle-Trigger.
