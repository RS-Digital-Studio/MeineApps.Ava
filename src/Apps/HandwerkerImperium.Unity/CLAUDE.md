...
| 3D-Asset-Import | **glTFast 6.12.0** (`com.unity.cloud.gltfast`, Apache-2.0, **embedded + gepatcht**) | GLB der ComfyUI-Pipeline → URP-Lit mit vollem PBR (Albedo+Metallic/Roughness) automatisch. NICHT FBX/OBJ (verlieren Metallic/Roughness). `Packages/manifest.json` ist git-ignored → Dependency hier dokumentiert. **Pflicht-Patches (Paket embedden — `Packages/com.unity.cloud.gltfast/`, lokal/git-ignored):** (1) glTFast #764, Android/IL2CPP: in `Runtime/Scripts/Animation/AnimationPlayableComponent.cs` das `using NUnit.Framework;` entfernen + beide `Assert.IsTrue` → `Debug.Assert` — sonst bricht der CIL-Linker mit `IL1005 … Failed to resolve 'nunit.framework'`. (2) Build-Größe: in `Editor/Scripts/GltfImporter.cs` vor `AddObjectToAsset(ctx, $"textures/…")` die Textur via `EditorUtility.CompressTexture` plattformgerecht komprimieren (Android/iOS ASTC_6x6, Desktop DXT5) — Sub-Asset-Texturen landen sonst UNKOMPRIMIERT im Player (44 GLBs = 1,4 GB Assets/505-MB-APK); nach dem Patch GLBs force-reimportieren. Registry bietet kein gefixtes 6.13 (nur bis 6.6.0-pre). Asset-Quelle/Re-Import → `Assets/_Project/Art/Models/README.md` |
