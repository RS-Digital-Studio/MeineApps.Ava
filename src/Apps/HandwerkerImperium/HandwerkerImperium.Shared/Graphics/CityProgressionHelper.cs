...
public static class CityProgressionHelper
{
    ...
    // Farbe wird vor jeder Verwendung gesetzt.
    private static readonly SKPaint _sidewalkPaint = new() { IsAntialias = true };
    private static readonly SKPaint _streetPaint = new() { IsAntialias = false };
    private static readonly SKPaint _curbPaint = new() { IsAntialias = false };
    private static readonly SKPaint _stripePaint = new() { IsAntialias = false };
    private static readonly SKPaint _fugenPaint = new() { IsAntialias = false };
    private static readonly SKPaint _gravelPaint = new() { IsAntialias = false };
    private static readonly SKPaint _stonePaint = new() { IsAntialias = true };
    private static readonly SKPaint _waterPaint = new() { IsAntialias = true };
    private static readonly SKPaint _treeTrunkPaint = new() { IsAntialias = true };
    private static readonly SKPaint _treeLeafPaint = new() { IsAntialias = true };
    private static readonly SKPaint _lanternPolePaint = new() { IsAntialias = true };
    private static readonly SKPaint _lanternGlowPaint = new() { IsAntialias = true };
    private static readonly SKPaint _benchPaint = new() { IsAntialias = true };
    private static readonly SKPaint _benchLegPaint = new() { IsAntialias = true };
    private static readonly SKPaint _flowerPaint = new() { IsAntialias = true };

    // Gecachter Path fuer Laternen-Lichtkegel (vermeidet Allokation pro Frame)
        ...
        float streetHeight, int worldTier, float nightDim, float time)
    {
        ...
        // Buergersteig (ab Tier 3)
        if (hasSidewalk)
        {
            _sidewalkPaint.Color = CityBuildingShapes.ApplyDim(new SKColor(0xBD, 0xB7, 0xAB), nightDim);
        ...
        // Strassenflaeche
        _streetPaint.Color = CityBuildingShapes.ApplyDim(streetColor, nightDim);
        canvas.DrawRect(bounds.Left, streetY, bounds.Width, streetHeight, _streetPaint);
        ...
        // Bordsteine
        _curbPaint.Color = CityBuildingShapes.ApplyDim(curbiColor, nightDim);
        canvas.DrawRect(bounds.Left, streetY, bounds.Width, 1.5f, _curbPaint);
        canvas.DrawRect(bounds.Left, streetY + streetHeight - 1.5f, bounds.Width, 1.5f, _curbPaint);

        // Mittelstreifen
        _stripePaint.Color = CityBuildingShapes.ApplyDim(stripeColor, nightDim);
        ...
        {
            canvas.DrawRect(sx, stripeY, dashLen, 1.5f, _stripePaint);
        }
    }
        ...
        int worldTier, float nightDim, float time)
    {
        ...
        float decoY = streetY - 6; // Ueber der Strasse, auf dem Buergersteig
        ...
        float time)
    {
        ...
        // Tier 6+: Zweite Krone oben (ueppiger Baum)
        if (worldTier >= 6)
        {
    ...
    private static void DrawLantern(SKCanvas canvas, float x, float y, float nightDim, float time)
    {
        ...
        // Cone-foermiger Lichtkegel nach unten (nachts)
        if (isNight)
        {
            ...
            canvas.DrawPath(_conePath, _lanternGlowPaint);
    ...
    {
        _treeLeafPaint.Color = CityBuildingShapes.ApplyDim(new SKColor(0x4C, 0x8C, 0x3C), nightDim);
        canvas.DrawOval(x, y - 2.5f, 4, 2.5f, _treeLeafPaint);

        // Dunklere Mitte fuer Tiefe
        _treeLeafPaint.Color = CityBuildingShapes.ApplyDim(new SKColor(0x38, 0x6B, 0x2C), nightDim);
    ...
    {
        // Becken (steingrau)
        _stonePaint.Color = CityBuildingShapes.ApplyDim(new SKColor(0x9E, 0x9E, 0x9E), nightDim);
        canvas.DrawOval(x, y - 2, 5, 2.5f, _stonePaint);

        // Wasser (blau, animiert)
        _waterPaint.Color = CityBuildingShapes.ApplyDim(new SKColor(0x42, 0xA5, 0xF5, 180), nightDim);
        canvas.DrawOval(x, y - 2.5f, 3.5f, 1.5f, _waterPaint);

        // Wasserfontaene (3 Tropfen steigen auf)
        for (int d = 0; d < 3; d++)
        {
            ...
            _waterPaint.Color = new SKColor(0x64, 0xB5, 0xF6, alpha);
            canvas.DrawCircle(dx, dy, 0.8f + (1f - progress) * 0.5f, _waterPaint);
        }
    }
        ...
        float streetHeight, float nightDim, bool isPremium)
    {
        ...
        _fugenPaint.Color = CityBuildingShapes.ApplyDim(
        ...
        // Horizontale Fugen
        canvas.DrawRect(bounds.Left, streetY + stoneH - 0.5f, bounds.Width, 1, _fugenPaint);

        // Vertikale Fugen (versetzt in zweiter Reihe)
        for (float sx = bounds.Left; sx < bounds.Right; sx += stoneW)
        {
            canvas.DrawRect(sx, streetY, 0.5f, stoneH, _fugenPaint);
            canvas.DrawRect(sx + stoneW / 2f, streetY + stoneH, 0.5f, stoneH, _fugenPaint);
        }
    }
        ...
        float streetHeight, float nightDim)
    {
        ...
        for (int g = 0; g < 25; g++)
        {
            ...
            _gravelPaint.Color = (hash % 2 == 0) ? lightGravel : darkGravel;
            canvas.DrawCircle(gx, gy, 0.8f + (hash % 50) / 100f, _gravelPaint);
        }
    }
}
