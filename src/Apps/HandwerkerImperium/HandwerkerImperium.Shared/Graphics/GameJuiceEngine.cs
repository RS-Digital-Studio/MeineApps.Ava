...
public sealed class GameJuiceEngine : IDisposable
{
    ...
    /// <summary>
    /// Wenn true, werden teure visuelle Effekte
    ...
    {
        // ReduceMotion daempft Shake auf 30%
    ...
    {
        if (ReduceMotion) return; // A11y-Friendly
    ...
    {
        // ReduceMotion reduziert Coins um 60% (4-6 statt 10),
    ...
    {
        if (ReduceMotion) { count = Math.Max(2, count / 3); duration *= 0.7f; } // A11y
    ...
    {
        // ReduceMotion = kein Confetti (Photosensitivity-Schutz).
