...
///
/// Optimiert: SKPaint-Instanzen als Felder statt pro-Frame Allokationen.
...
public sealed class InventGameRenderer : IDisposable
{
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // WIEDERVERWENDBARE PAINTS - Statisch (konstante Properties)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // linePaint: PathEffect aendert sich (animierter Dash-Offset) → Instanz-Feld
    ...
    private static readonly SKPaint _tileShadowPaint = new()
    {
        ...
        IsAntialias = true,
        MaskFilter = _blur4Filter
    };

    // --- DrawTileBorder: Completed-Border ---
    private static readonly SKPaint _completedGlowBorderPaint = new()
    {
    ...
    // --- DrawTileBorder: Standard-Border ---
    ...
    // --- DrawCheckmark ---
    private static readonly SKPaint _checkCircleGlowPaint = new()
    {
        Color = CompletedGreen.WithAlpha(40),
        IsAntialias = true,
        MaskFilter = _blur3Filter
    };
    private static readonly SKPaint _checkCircleFillPaint = new()
    {
        Color = CompletedGreen, IsAntialias = true
    };
    private static readonly SKPaint _checkmarkStrokePaint = new()
    {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // WIEDERVERWENDBARE PAINTS - Instanz (dynamische Farben pro Frame)
    // ═══════════════════════════════════════════════════════════════════════

    // Hintergrund-Paint fuer Shader (Farbe/Shader aendert sich)
    private readonly SKPaint _bgShaderPaint = new() { IsAntialias = false };
    // dotPaint fuer pulsierende Kreuzungspunkte (Alpha aendert sich)
    private readonly SKPaint _dotPaint = new() { IsAntialias = true };
    // cornerPaint fuer Eck-Markierungen (Alpha aendert sich)
    ...
    // Kachel-Hintergrund + Highlight (Shader aendert sich pro Kachel)
    private readonly SKPaint _tileGradientPaint = new() { IsAntialias = true };
    private readonly SKPaint _tileHighlightPaint = new() { IsAntialias = true };

    // DrawTileBorder: Aktiver Neon-Rahmen (Alpha aendert sich)
    ...
    private readonly SKPaint _activeBorderPaint = new()
    {
        IsAntialias = true,
        Style = SKPaintStyle.Stroke,
        StrokeWidth = 2.5f
    };
    // DrawTileBorder: Fehler-Rand (Alpha aendert sich)
    ...
    // DrawErrorOverlay (Alpha aendert sich)
    private readonly SKPaint _errorOverlayPaint = new() { IsAntialias = true };
    private readonly SKPaint _boltPaint = new()
    {
        IsAntialias = true,
        StrokeWidth = 2.5f,
        Style = SKPaintStyle.Stroke,
    ...
    // Partikel-Paint (Farbe aendert sich pro Partikel)
    ...
    // Fortschrittsbalken (Farbe aendert sich)
    ...
    // DrawTileNumber (Farbe/Groesse aendert sich)
    ...
    // --- DrawGearIcon ---
    private static readonly SKPaint _gearBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x60, 0x7D, 0x8B), StrokeWidth = 1.5f
    };
    private static readonly SKPaint _gearToothPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x90, 0xA4, 0xAE)
    };
    private static readonly SKPaint _gearHolePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x15, 0x0B, 0x35)
    };
    // gearFillPaint: hat Shader → Instanz-Paint + separater Shader pro Aufruf
    private static readonly SKPaint _gearFillPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill
    };

    // --- DrawPistonIcon ---
    private static readonly SKPaint _pistonRingPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x61, 0x61, 0x61), StrokeWidth = 2
    };
    private static readonly SKPaint _pistonRodPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x78, 0x90, 0x9C)
    };
    private static readonly SKPaint _pistonBoltPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xB0, 0xBE, 0xC5)
    };
    private static readonly SKPaint _pistonBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x54, 0x6E, 0x7A), StrokeWidth = 1.5f
    };
    private static readonly SKPaint _pistonHeadPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill
    };

    // --- DrawWireIcon ---
    private static readonly SKPaint _wirePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xEF, 0x53, 0x50), StrokeWidth = 3, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _wirePlugPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xFF, 0xCA, 0x28)
    };

    // --- DrawBoardIcon ---
    private static readonly SKPaint _boardFillPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x2E, 0x7D, 0x32)
    };
    private static readonly SKPaint _boardTracePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xFF, 0xD5, 0x4F, 0xC0), StrokeWidth = 1.5f
    };
    private static readonly SKPaint _boardChipPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x21, 0x21, 0x21)
    };
    private static readonly SKPaint _boardBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
    ...
    // --- DrawScrewIcon ---
    private static readonly SKPaint _screwBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x78, 0x90, 0x9C), StrokeWidth = 2
    };
    private static readonly SKPaint _screwSlotPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x54, 0x6E, 0x7A), StrokeWidth = 2.5f, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _screwHeadPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill
    };

    // --- DrawHousingIcon ---
    private static readonly SKPaint _housingBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x45, 0x5A, 0x64), StrokeWidth = 2
    };
    private static readonly SKPaint _housingScrewPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xB0, 0xBE, 0xC5)
    };
    private static readonly SKPaint _housingSlitPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x45, 0x5A, 0x64, 0x80), StrokeWidth = 1.5f
    };
    private static readonly SKPaint _housingFillPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill
    };

    // --- DrawSpringIcon ---
    private static readonly SKPaint _springPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
    ...
    // --- DrawLensIcon ---
    private static readonly SKPaint _lensFillPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x42, 0xA5, 0xF5, 0xA0)
    };
    private static readonly SKPaint _lensBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x1E, 0x88, 0xE5), StrokeWidth = 2
    };
    private static readonly SKPaint _lensReflexPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
    ...
    // --- DrawMotorIcon ---
    private static readonly SKPaint _motorCoilPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xFF, 0x8F, 0x00), StrokeWidth = 2
    };
    private static readonly SKPaint _motorAxlePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xB0, 0xBE, 0xC5), StrokeWidth = 3, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _motorBearingPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xE0, 0xE0, 0xE0)
    };
    private static readonly SKPaint _motorBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x54, 0x6E, 0x7A), StrokeWidth = 1.5f
    };
    private static readonly SKPaint _motorBodyPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill
    };

    // --- DrawBatteryIcon ---
    private static readonly SKPaint _batteryBodyPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x43, 0xA0, 0x47)
    };
    private static readonly SKPaint _batteryCapPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xB0, 0xBE, 0xC5)
    };
    private static readonly SKPaint _batterySymbolPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = SKColors.White, StrokeWidth = 2, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _batteryBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
    ...
    // --- DrawSwitchIcon ---
    private static readonly SKPaint _switchBasePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x78, 0x90, 0x9C)
    };
    private static readonly SKPaint _switchPivotPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xB0, 0xBE, 0xC5)
    };
    private static readonly SKPaint _switchLeverPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xFF, 0xCA, 0x28), StrokeWidth = 3, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _switchHandlePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xFF, 0xD5, 0x4F)
    };
    private static readonly SKPaint _switchContactPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0xE0, 0xE0, 0xE0)
    };
    private static readonly SKPaint _switchBorderPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0x54, 0x6E, 0x7A), StrokeWidth = 1.5f
    };
    ...
    // --- DrawAntennaIcon ---
    private static readonly SKPaint _antennaStickPaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        Color = new SKColor(0xB0, 0xBE, 0xC5), StrokeWidth = 2.5f, StrokeCap = SKStrokeCap.Round
    };
    private static readonly SKPaint _antennaBasePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Fill,
        Color = new SKColor(0x78, 0x90, 0x9C)
    };
    private static readonly SKPaint _antennaWavePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        StrokeWidth = 1.5f
    };
    ...
    // --- DrawDefaultIcon ---
    private static readonly SKPaint _defaultCirclePaint = new()
    {
        IsAntialias = true, Style = SKPaintStyle.Stroke,
        ...
        float deltaTime)
    {
        ...
        {
            _background ??= _assetService.GetBitmap("minigames/invent_bg.webp");
            if (_background == null)
                _ = _assetService.LoadBitmapAsync("minigames/invent_bg.webp");
            if (_background != null)
    ...
    private void DrawLabBackground(SKCanvas canvas, SKRect bounds)
    {
        ...
        _dotPaint.Color = NeonViolet.WithAlpha((byte)(25 * pulse));
        for (float x = bounds.Left; x < bounds.Right; x += 80)
        {
            for (float y = bounds.Top; y < bounds.Bottom; y += 80)
            {
        ...
        _cornerPaint.Color = NeonViolet.WithAlpha(40);
        float cs = 25;
        // Oben links
        canvas.DrawLine(bounds.Left + 4, bounds.Top + 4, bounds.Left + 4 + cs, bounds.Top + 4, _cornerPaint);
        canvas.DrawLine(bounds.Left + 4, bounds.Top + 4, bounds.Left + 4, bounds.Top + 4 + cs, _cornerPaint);
        // Oben rechts
        canvas.DrawLine(bounds.Right - 4, bounds.Top + 4, bounds.Right - 4 - cs, bounds.Top + 4, _cornerPaint);
        canvas.DrawLine(bounds.Right - 4, bounds.Top + 4, bounds.Right - 4, bounds.Top + 4 + cs, _cornerPaint);
        // Unten links
        canvas.DrawLine(bounds.Left + 4, bounds.Bottom - 4, bounds.Left + 4 + cs, bounds.Bottom - 4, _cornerPaint);
        canvas.DrawLine(bounds.Left + 4, bounds.Bottom - 4, bounds.Left + 4, bounds.Bottom - 4 - cs, _cornerPaint);
        // Unten rechts
        canvas.DrawLine(bounds.Right - 4, bounds.Bottom - 4, bounds.Right - 4 - cs, bounds.Bottom - 4, _cornerPaint);
        canvas.DrawLine(bounds.Right - 4, bounds.Bottom - 4, bounds.Right - 4, bounds.Bottom - 4 - cs, _cornerPaint);
    }
    ...
    private void DrawTileBorder(SKCanvas canvas, SKRect rect, InventPartData part, float cr)
    {
        if (part.IsActive && !part.IsCompleted)
        {
            ...
            // Aeusserer Glow
            _activeOuterGlowPaint.Color = NeonViolet.WithAlpha((byte)(50 * pulse));
            ...
            // Innerer Neon-Rand
            _activeBorderPaint.Color = NeonViolet.WithAlpha(alpha);
            canvas.DrawRoundRect(rect, cr, cr, _activeBorderPaint);
        ...
        else if (part.IsCompleted)
        {
            ...
            canvas.DrawRoundRect(rect, cr, cr, _completedGlowBorderPaint);
    ...
    private static void DrawCircuitCorners(SKCanvas canvas, SKRect rect, SKColor color, float cr)
    {
        ...
        // Farbe aendert sich pro Aufruf → Color direkt setzen
    ...
    private void DrawErrorOverlay(SKCanvas canvas, SKRect rect, float cr)
    {
        ...
        canvas.DrawPath(_cachedPath, _boltPaint);
    }
    ...
    private void DrawCompletionFlash(SKCanvas canvas, SKRect bounds, float deltaTime)
    {
        ...
        using var flashShader = SKShader.CreateRadialGradient(
            ...
            null, SKShaderTileMode.Clamp);
        _flashShaderPaint.Shader = flashShader;
        canvas.DrawRect(bounds, _flashShaderPaint);
        _flashShaderPaint.Shader = null;
        ...
        int completed, int total, float y)
    {
        ...
        if (progress > 0)
        {
            ...
            // Glow am Ende
            _progressGlowPaint.Color = fillColor.WithAlpha(60);
    ...
    private void UpdateAndDrawSparks(SKCanvas canvas, float deltaTime)
    {
        for (int i = _sparkCount - 1; i >= 0; i--)
        {
            ...
            if (s.IsGolden)
            {
                ...
                _sparkPaint.Color = new SKColor(s.R, s.G, (byte)(s.B * shimmer), (byte)(alpha * 220));
            }
            else
            {
                _sparkPaint.Color = new SKColor(s.R, s.G, s.B, (byte)(alpha * 200));
            }

            float size = s.Size * (1 - lifeRatio * 0.5f);
            canvas.DrawCircle(s.X, s.Y, size, _sparkPaint);

            // Heller Kern
            _sparkPaint.Color = new SKColor(0xFF, 0xFF, 0xFF, (byte)(alpha * 100));
            canvas.DrawCircle(s.X, s.Y, size * 0.4f, _sparkPaint);
        }
    }
    ...
    private void UpdateAndDrawCircuitPulses(SKCanvas canvas, InventPartData[] parts, float deltaTime)
    {
        for (int i = _pulseCount - 1; i >= 0; i--)
        {
            ...
            // Leuchtender Punkt
            _pulsePaint.Color = new SKColor(p.R, p.G, p.B, (byte)(alpha * 180));
            canvas.DrawCircle(px, py, 5, _pulsePaint);

            // Heller Kern
            _pulsePaint.Color = new SKColor(0xFF, 0xFF, 0xFF, (byte)(alpha * 120));
            canvas.DrawCircle(px, py, 2, _pulsePaint);
        }
    }
    ...
    private void UpdateAndDrawAmbient(SKCanvas canvas, SKRect bounds, float deltaTime)
    {
        ...
        for (int i = _ambientCount - 1; i >= 0; i--)
        {
            ...
            _ambientPaint.Color = new SKColor(a.R, a.G, a.B, (byte)(alpha * 50));
            canvas.DrawCircle(a.X, a.Y, a.Size, _ambientPaint);
        }
    }
    ...
    private static void DrawGearIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        for (int i = 0; i < teeth; i++)
        {
            ...
            canvas.DrawRect(-toothW / 2, -toothW / 2, toothW, toothW, _gearToothPaint);
            canvas.Restore();
        }

        canvas.DrawCircle(cx, cy, outerR, _gearBorderPaint);

        float innerR = half * 0.55f;
        canvas.DrawCircle(cx, cy, innerR * 0.45f, _gearHolePaint);
        canvas.DrawCircle(cx, cy, innerR * 0.45f, _gearBorderPaint);
    }
    ...
    private static void DrawPistonIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawLine(headRect.Left + 3, headRect.Top + half * 0.15f, headRect.Right - 3, headRect.Top + half * 0.15f, _pistonRingPaint);

        var rodRect = SKRect.Create(cx - half * 0.15f, cy, half * 0.3f, half * 0.8f);
        canvas.DrawRect(rodRect, _pistonRodPaint);

        canvas.DrawCircle(cx, cy + half * 0.8f, half * 0.18f, _pistonBoltPaint);

        canvas.DrawRoundRect(headRect, 3, 3, _pistonBorderPaint);
    }
    ...
    private static void DrawBoardIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawRoundRect(boardRect, 3, 3, _boardFillPaint);

        canvas.DrawLine(boardRect.Left + 4, cy - half * 0.2f, boardRect.Right - 4, cy - half * 0.2f, _boardTracePaint);
        canvas.DrawLine(boardRect.Left + 4, cy + half * 0.2f, boardRect.Right - 4, cy + half * 0.2f, _boardTracePaint);
        canvas.DrawLine(cx - half * 0.3f, boardRect.Top + 4, cx - half * 0.3f, boardRect.Bottom - 4, _boardTracePaint);
        canvas.DrawLine(cx + half * 0.3f, boardRect.Top + 4, cx + half * 0.3f, boardRect.Bottom - 4, _boardTracePaint);

        canvas.DrawRect(cx - half * 0.2f, cy - half * 0.15f, half * 0.4f, half * 0.3f, _boardChipPaint);

        canvas.DrawRoundRect(boardRect, 3, 3, _boardBorderPaint);
    }
    ...
    private static void DrawScrewIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawCircle(cx, cy, radius, _screwBorderPaint);

        float slotLen = radius * 0.6f;
        canvas.DrawLine(cx - slotLen, cy, cx + slotLen, cy, _screwSlotPaint);
        canvas.DrawLine(cx, cy - slotLen, cx, cy + slotLen, _screwSlotPaint);
    }
    ...
    private static void DrawHousingIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawRoundRect(housingRect, 6, 6, _housingBorderPaint);

        float screwR = half * 0.08f;
        float inset = half * 0.2f;
        canvas.DrawCircle(housingRect.Left + inset, housingRect.Top + inset, screwR, _housingScrewPaint);
        canvas.DrawCircle(housingRect.Right - inset, housingRect.Top + inset, screwR, _housingScrewPaint);
        canvas.DrawCircle(housingRect.Left + inset, housingRect.Bottom - inset, screwR, _housingScrewPaint);
        canvas.DrawCircle(housingRect.Right - inset, housingRect.Bottom - inset, screwR, _housingScrewPaint);

        for (int i = 0; i < 3; i++)
        {
            float ly = cy - half * 0.2f + i * half * 0.2f;
            canvas.DrawLine(cx - half * 0.3f, ly, cx + half * 0.3f, ly, _housingSlitPaint);
        }
    }
    ...
    private static void DrawSpringIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawLine(lastX, bottomY, cx, bottomY, _springPaint);
        canvas.DrawLine(cx - half * 0.15f, bottomY, cx + half * 0.15f, bottomY, _springPaint);
    }
    ...
    private static void DrawMotorIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawLine(bodyRect.Left + 4, cy - half * 0.15f, bodyRect.Right - 4, cy - half * 0.15f, _motorCoilPaint);
        canvas.DrawLine(bodyRect.Left + 4, cy + half * 0.15f, bodyRect.Right - 4, cy + half * 0.15f, _motorCoilPaint);

        canvas.DrawLine(bodyRect.Right, cy, bodyRect.Right + half * 0.35f, cy, _motorAxlePaint);

        canvas.DrawCircle(bodyRect.Right + half * 0.35f, cy, half * 0.1f, _motorBearingPaint);

        canvas.DrawRoundRect(bodyRect, 4, 4, _motorBorderPaint);
    }
    ...
    private static void DrawBatteryIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawRoundRect(bodyRect, 3, 3, _batteryBodyPaint);

        canvas.DrawRect(cx - half * 0.2f, bodyRect.Top - half * 0.15f, half * 0.4f, half * 0.15f, _batteryCapPaint);

        float plusY = cy - half * 0.3f;
        canvas.DrawLine(cx - half * 0.15f, plusY, cx + half * 0.15f, plusY, _batterySymbolPaint);
        canvas.DrawLine(cx, plusY - half * 0.15f, cx, plusY + half * 0.15f, _batterySymbolPaint);

        float minusY = cy + half * 0.3f;
        canvas.DrawLine(cx - half * 0.15f, minusY, cx + half * 0.15f, minusY, _batterySymbolPaint);

        canvas.DrawRoundRect(bodyRect, 3, 3, _batteryBorderPaint);
    }
    ...
    private static void DrawSwitchIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        canvas.DrawRoundRect(baseRect, 4, 4, _switchBasePaint);

        float pivotY = baseRect.Top;
        canvas.DrawCircle(cx, pivotY, half * 0.12f, _switchPivotPaint);

        canvas.DrawLine(cx, pivotY, cx + half * 0.5f, cy - half * 0.6f, _switchLeverPaint);

        canvas.DrawCircle(cx + half * 0.5f, cy - half * 0.6f, half * 0.15f, _switchHandlePaint);

        canvas.DrawCircle(baseRect.Left + half * 0.2f, pivotY, half * 0.08f, _switchContactPaint);
        canvas.DrawCircle(baseRect.Right - half * 0.2f, pivotY, half * 0.08f, _switchContactPaint);

        canvas.DrawRoundRect(baseRect, 4, 4, _switchBorderPaint);
    }
    ...
    private static void DrawAntennaIcon(SKCanvas canvas, float cx, float cy, float size)
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            ...
            _antennaWavePaint.Color = new SKColor(0xBB, 0x86, 0xFC, alpha);
            canvas.DrawArc(
                SKRect.Create(cx - waveR, cy - half * 0.5f - waveR, waveR * 2, waveR * 2),
                -150, 120, false, _antennaWavePaint);
        }
    }
    ...
    private static void DrawCheckmark(SKCanvas canvas, SKRect rect)
    {
        ...
        canvas.DrawCircle(cx, cy, badgeSize / 2 + 2, _checkCircleGlowPaint);
        canvas.DrawCircle(cx, cy, badgeSize / 2, _checkCircleFillPaint);

        float s = badgeSize * 0.22f;
        canvas.DrawLine(cx - s * 1.2f, cy, cx - s * 0.1f, cy + s, _checkmarkStrokePaint);
        canvas.DrawLine(cx - s * 0.1f, cy + s, cx + s * 1.5f, cy - s * 0.8f, _checkmarkStrokePaint);
    }

    /// <summary>
    /// Gibt native SkiaSharp-Ressourcen frei.
    /// Statische Paints/MaskFilter werden NICHT disposed (leben fuer die gesamte App-Dauer).
    /// </summary>
    public void Dispose()
    {
        if (_disposed) return;
