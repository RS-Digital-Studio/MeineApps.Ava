...
/// <summary>
/// Strategie, mit der ein Auftrag angegangen wird (v2.0.35).
///
/// Orthogonal zu <see cref="OrderDifficulty"/> — Difficulty ist am Auftrag fest,
/// Strategy waehlt der Spieler vor jedem MiniGame-Start.
///
/// Effekt:
/// - <c>Safe</c>: Weniger Reward, aber das MiniGame wird deutlich leichter (+50% Zonen, +30% Zeit).
/// - <c>Standard</c>: Baseline — keine Aenderung zu vorherigem Verhalten.
/// - <c>Risk</c>: Deutlich mehr Reward bei Erfolg, aber das MiniGame wird knallhart
///   (-50% Zonen, +30% Geschwindigkeit). Miss bedeutet <b>kein Reward + Reputation-Hit</b>.
///
/// Wirtschaftliche Balance: Im Schnitt (bei gut spielenden Spielern) ist das Einkommen
/// ungefaehr gleich — Safe gibt zuverlaessig wenig, Risk mit Glueck/Skill deutlich mehr.
/// Psychologisch: Jeder Auftrag wird zur aktiven Entscheidung.
/// </summary>
...
/// <summary>
/// Extension-Methoden fuer <see cref="OrderStrategy"/> — Multiplikatoren fuer Reward,
/// MiniGame-Parameter und Penalty-Werte bei Miss.
/// </summary>
public static class OrderStrategyExtensions
{
    /// <summary>
    /// Reward-Multiplikator. Wirkt multiplikativ auf <c>BaseReward * Difficulty * OrderType</c>.
    /// </summary>
    ...
    /// <summary>
    /// XP-Multiplikator analog zum Reward-Multiplikator.
    /// </summary>
    ...
    /// <summary>
    /// Zeitmultiplikator fuer MiniGames mit festem Timer (Pipe-Puzzle, Wiring, Invent, etc.).
    ...
    /// <summary>
    /// Ob bei dieser Strategie ein Miss den Auftrag komplett scheitern laesst.
    ...
    {
        OrderStrategy.Risk => -10,           // Spuerbar, aber nicht ruinoes
        _ => 0
    };

    /// <summary>Lokalisierungs-Key fuer den Strategy-Namen.</summary>
    public static string GetNameKey(this OrderStrategy strategy) => strategy switch
    {
        OrderStrategy.Safe => "OrderStrategySafe",
        OrderStrategy.Standard => "OrderStrategyStandard",
        OrderStrategy.Risk => "OrderStrategyRisk",
        _ => "OrderStrategyStandard"
    };

    /// <summary>Lokalisierungs-Key fuer die Beschreibung.</summary>
    public static string GetDescriptionKey(this OrderStrategy strategy) => strategy switch
    {
        OrderStrategy.Safe => "OrderStrategySafeDesc",
        OrderStrategy.Standard => "OrderStrategyStandardDesc",
        OrderStrategy.Risk => "OrderStrategyRiskDesc",
        _ => "OrderStrategyStandardDesc"
    };
}
