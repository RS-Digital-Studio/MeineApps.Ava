...
| `CraftingInventory`/`ReservedInventory`/`ActiveCraftingJobs` racen mit AutoSave-Serializer | Separate Locks (`_craftingLock`/`_warehouseLock`) oder lockfrei schliessen NICHT gegen den Serializer (Background-Thread unter `_stateLock`) aus | ALLE Mutationen UND enumerierenden Reads dieser Collections (CraftingService, WarehouseService, GameLoop-Tick) ueber `_gameStateService.ExecuteWithLock` — KEINE eigenen Locks. Events/AddMoney/Analytics NACH dem Lock |
| Geteilte Firebase-Objekte (Co-op/Auktion/Mega-Projekt) brechen zwischen echten Spielern | HMAC mit GERAETE-lokalem Key (Installations-GUID) -> Cross-Client-Validierung scheitert immer | `ComputeStringHmac` nutzt den geteilten App-Salt-Key (`_sharedHmacKey`, geraete-uebergreifend identisch). Der geraete-lokale Key bleibt NUR fuer die lokale Save-Signatur (`ComputeSignature`/`VerifySignature`) |
| Geteilte Firebase-Zaehler verlieren parallele Schreiber | read-modify-write auf geteiltem Key (`weeklyProgress`, Mega-Projekt `contributions`) -> Last-Write-Wins | Server-seitiges atomares `IFirebaseService.IncrementAsync` (Firebase ServerValue increment) statt absolutem PATCH-Wert. Per-Spieler-Subpfade (`guild_members/.../contribution`, `donations/{playerId}/...`) sind konfliktfrei |
| Auktion settelt falschen Gewinner / Re-Spend bei Gilden-Writes | Settle/Level-Write auf geteiltem Knoten ohne Master-Guard bzw. ohne `SetAsync`-Rueckgabe-Pruefung | Settle nur als Master (`IsMasterClientAsync`) + Overdue-Fallback; `SetAsync`-bool pruefen und bei `false` Kosten refunden / lokalen ++ zuruecknehmen |
