...
/// <summary>
/// Reine, abhängigkeitsfreie Feiertagsberechnung (DE 16 Bundesländer, AT 9 Bundesländer,
/// CH 12 Kantone). Bewusst ohne DB-/Service-Abhängigkeit, damit sowohl <see cref="HolidayService"/>
/// (mit Cache) als auch <see cref="DatabaseService"/>.IsHolidayAsync sie nutzen können, ohne einen
/// DI-Zyklus zu erzeugen. Setzt Region/Year auf allen berechneten Einträgen, sodass die Werte
/// für Persistenz und Filter konsistent sind.
/// </summary>
public static class HolidayCalculator
{
    /// <summary>
    /// Berechnet alle Feiertage eines Jahres für eine Region (Region/Year werden gesetzt).
    /// </summary>
    public static List<HolidayEntry> Calculate(int year, string region)
    {
    ...
    /// <summary>
    /// Prüft (in-memory), ob ein Datum in der Region ein Feiertag ist.
    /// </summary>
    public static bool IsHoliday(DateTime date, string region)
    {
        var d = date.Date;
        return Calculate(date.Year, region).Any(h => h.Date.Date == d);
    }
