...
| `PomodoroStatisticsVisualization.cs` | Monats-Heatmap (GitHub-Contributions-Stil): 7×5 Grid, 5 Intensitätsstufen, Wochentag-Labels, Heute-Highlight. `HitTest(bounds, x, y, totalDays, weekDayLabels)` berechnet den Tages-Index anhand der Grid-Geometrie (kein Canvas.LocalClipBounds). Statisch. |
...
(`_lastW`/`_lastH`-Vergleich). `Dispose()` löscht alle Paints + Filter + Shader.
