...
public class LiveTradingManager : IDisposable
{
    ...
    public async Task StartAsync(string strategyName)
    {
        ...
        // Equity-Snapshots. MUSS nach _service.Start() laufen — erst dort wird der RiskManager via
        // StartBase(new RiskManager(...)) erzeugt; davor ist _service.RiskManager null. Ohne diese
        // Rehydration resettet jeder Engine-Restart den Daily-Loss-Circuit + Total-Drawdown-Schutz
        // auf 0 (amnesisch). PnL ist konto-, nicht strategiebezogen → Strategiewechsel hier irrelevant.
        if (_dbService != null)
        {
            try
            {
